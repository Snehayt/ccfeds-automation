...
export default class CmpBannerPage {
  constructor(page) {
    this.page = page;
  ...
  // ── Accept All — clicks Enable All, verifies banner disappears ──────────
  async validateEnableAll() {
    await this.enableAllCTA.waitFor({ state: 'visible', timeout: 10000 });
    await this.enableAllCTA.click();
    await this.bannerContainer.waitFor({ state: 'hidden', timeout: 10000 });
    console.info('[CMP] Banner dismissed via Enable All ✓');
  }

  // ── Reject All — EU only, clicks Don't Enable, verifies banner disappears
  async validateDontEnable() {
    await this.dontEnableCTA.waitFor({ state: 'visible', timeout: 10000 });
    await this.dontEnableCTA.click();
    await this.bannerContainer.waitFor({ state: 'hidden', timeout: 10000 });
    console.info('[CMP] Banner dismissed via Don\'t Enable ✓');
  }

  // ── Consent persistence — banner reappears after reload (pre-consent) ───
  async validateBannerPersistencePreConsent() {
    await this.page.reload();
    await this.page.waitForLoadState('domcontentloaded');
    await this.bannerContainer.waitFor({ state: 'visible', timeout: 15000 });
    await expect(this.bannerContainer).toBeVisible();
    console.info('[CMP] Consent persistence (pre): banner reappeared after reload ✓');
  }

  // ── Consent persistence — banner gone after accepting ────────────────────
  async validateBannerPersistencePostConsent() {
    await this.page.reload();
    await this.page.waitForLoadState('domcontentloaded');
    await expect(this.bannerContainer).not.toBeVisible();
    console.info('[CMP] Consent persistence (post): banner not shown after consent ✓');
  }

  // ── Cookie groups — pre-consent: only C0001 active ───────────────────────
  async assertCookieGroupsPreConsent() {
    const groups = await this.page.evaluate(() => window.adobePrivacy?.activeCookieGroups?.() || []);
    expect(Array.isArray(groups), 'adobePrivacy.activeCookieGroups() must return array').toBe(true);
    expect(groups.includes('C0001'), 'C0001 (Strictly Necessary) must be active').toBe(true);
    expect(groups.includes('C0002'), 'C0002 (Performance) must NOT be active pre-consent').toBe(false);
    expect(groups.includes('C0003'), 'C0003 (Functional) must NOT be active pre-consent').toBe(false);
    expect(groups.includes('C0004'), 'C0004 (Targeting) must NOT be active pre-consent').toBe(false);
    console.info(`[CMP] Cookie groups pre-consent: ${groups.join(', ')} ✓`);
  }

  // ── Cookie groups — after reject: only C0001 active, C0002–C0004 blocked ─
  async assertCookieGroupsAfterReject() {
    const groups = await this.page.evaluate(() => window.adobePrivacy?.activeCookieGroups?.() || []);
    expect(Array.isArray(groups), 'adobePrivacy.activeCookieGroups() must return array').toBe(true);
    expect(groups.includes('C0001'), 'C0001 (Strictly Necessary) must stay active after reject').toBe(true);
    expect(groups.includes('C0002'), 'C0002 (Performance) must NOT be active after reject').toBe(false);
    expect(groups.includes('C0003'), 'C0003 (Functional) must NOT be active after reject').toBe(false);
    expect(groups.includes('C0004'), 'C0004 (Targeting) must NOT be active after reject').toBe(false);
    console.info(`[CMP] Cookie groups after reject: ${groups.join(', ')} — non-essential blocked ✓`);
  }

  // ── Cookie groups — post-consent: all groups C0001–C0004 active ──────────
  async assertCookieGroupsPostConsent() {
    const groups = await this.page.evaluate(() => window.adobePrivacy?.activeCookieGroups?.() || []);
    expect(Array.isArray(groups), 'adobePrivacy.activeCookieGroups() must return array').toBe(true);
    expect(groups.includes('C0001'), 'C0001 must be active post-consent').toBe(true);
    expect(groups.includes('C0002'), 'C0002 must be active post-consent').toBe(true);
    expect(groups.includes('C0003'), 'C0003 must be active post-consent').toBe(true);
    expect(groups.includes('C0004'), 'C0004 must be active post-consent').toBe(true);
    console.info(`[CMP] Cookie groups post-consent: ${groups.join(', ')} ✓`);
  }
