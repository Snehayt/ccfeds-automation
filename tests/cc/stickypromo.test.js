...
test.describe('verify sticky promo feature woring, scroll feature, CTAs working features', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('sticky promo displayed in page bottom', async () => {
      ...
      await expect(stickypromo.stickyPromoSection).toBeVisible();
      await expect(stickypromo.asideBlockStickiness).toBeVisible();
      await expect(stickypromo.bodyActionArea).toBeVisible();
      await expect(stickypromo.freeTrialButton).toBeVisible();
      await expect(stickypromo.buyNowButton).toBeVisible();
    });
  });
