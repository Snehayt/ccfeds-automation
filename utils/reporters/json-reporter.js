...
class JSONReporter extends BaseReporter {
  ...
  async persistData() {
    ...
    if (process.env.SUITE)       persistedObject.suite       = process.env.SUITE;
    if (process.env.DEVICE)      persistedObject.device      = process.env.DEVICE;
    if (process.env.LOCALE)      persistedObject.locale      = process.env.LOCALE;
    if (process.env.TRIGGERED_BY) persistedObject.triggeredBy = process.env.TRIGGERED_BY;
